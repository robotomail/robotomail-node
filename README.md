# Robotomail SDK for TypeScript and JavaScript
...
The runnable example in `examples/list-mailboxes.mjs` lists mailboxes and does not send email. Create an account and verify your email first. The Free plan includes one mailbox and 10 sends plus 10 receives per calendar month, restricted to your verified email address. Upgrade when you need more.
...
const events = mail.streamEvents({ mailboxId }, { signal: controller.signal });
for await (const frame of events) {
  if (frame.id !== undefined) lastEventId = frame.id;
  if (frame.event === "reconnect") break;
  console.log(frame.event, JSON.parse(frame.data));
}
// Reopen with { mailboxId, "Last-Event-ID": lastEventId } to resume.
// Breaking the loop closes the response; controller.abort() also cancels it.
```
